...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderLogs(logs) {
        ...
        logs.forEach((log, i) => {
          ...
          <td class="px-4 py-2 text-sm">${log.meta?.container_name || ""}</td>
          <td class="px-4 py-2 text-sm font-mono whitespace-pre-wrap break-words">${sanitize(log.message)}</td>
        ...
        `;
          ...
          <td colspan="7" class="px-4 py-2 text-sm text-gray-600 dark:text-gray-300">
            <div class="mb-2">
              ...
              <pre class="bg-gray-100 dark:bg-gray-800 p-2 rounded text-xs font-mono whitespace-pre-wrap">${sanitize(log.message)}</pre>
            </div>
