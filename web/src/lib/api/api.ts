...
export class MetricsApi {
	...
	async getMetricData(namespace: string, subNamespace: string, metric: string, params: any) {
		...
		return this.api.request(
			`/metrics/namespaces/${namespace}/${subNamespace}/${metric}/data${query ? `?${query}` : ''}`
		);
	}
	...
	async getUserPreferences() {
		try {
			...
			return {
				...
				notifications: {
					email_alerts: data.notifications?.email_alerts ?? false,
					push_alerts: data.notifications?.push_alerts ?? false,
					alert_frequency: data.notifications?.alert_frequency || ''
				},
				dashboard: {
					refresh_interval: data.dashboard?.refresh_interval ?? 30,
					default_time_range: data.dashboard?.default_time_range || '1h',
					show_system_metrics: data.dashboard?.show_system_metrics ?? true
				}
			};
		} catch (error) {
...
export class ApiClient {
	...
	async request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
		...
		if (!response.ok) {
			...
			throw new GoSightApiError(errorMessage || `HTTP ${response.status}`, response.status);
		}
