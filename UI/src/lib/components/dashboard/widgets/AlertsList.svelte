<script lang="ts">
	import type { Widget, WidgetData, Alert } from '$lib/types/dashboard';
	...
	// Setup real-time alerts subscription
	function setupRealTimeSubscription() {
		// Subscribe to live alerts store
		unsubscribe = dataService.liveAlerts.subscribe((liveAlerts) => {
			if (liveAlerts.length > 0) {
				// Apply widget filters if configured
				const filteredAlerts = applyFilters(liveAlerts);
				alerts = filteredAlerts.slice(0, widget.config?.limit || 10);
			}
		});
	}

	// Apply widget-specific filters
	function applyFilters(allAlerts: Alert[]): Alert[] {
		const { severity, endpointId, status } = widget.config || {};

		return allAlerts.filter((alert) => {
			if (severity && alert.severity !== severity) return false;
			if (endpointId && alert.endpoint_id !== endpointId) return false;
		...
		{#if widget.config?.showTitle !== false}
			...
			>
				...
				<div class="flex items-center gap-2">
					...
					{/if}
					<Badge color="gray" class="text-xs">{alerts.length}</Badge>
				</div>
			</div>
		{/if}
		...
		{:else if alerts.length === 0}
			...
			<div class="flex flex-1 items-center justify-center">
				<div class="text-center">
					...
					<div class="text-sm text-gray-500 dark:text-gray-400">No alerts</div>
				</div>
			</div>
		{:else}
			<!-- Alerts List -->
					...
					>
						<div class="flex items-start gap-2">
							...
							<div class="min-w-0 flex-1">
								...
								>
									<span>{alert.source || alert.endpoint_id || 'Unknown source'}</span>
									<span>{formatTimestamp(alert.timestamp || alert.created_at || new Date())}</span>
								</div>
							</div>
						</div>
					</div>
