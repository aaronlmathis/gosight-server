...
func SetupCommandsRoutes(r *mux.Router, commandsHandler *handlers.CommandsHandler, withAccessLog func(http.Handler) http.Handler) {
	...
	withAuth := gosightauth.AuthMiddleware(commandsHandler.Sys.Stores.Users)
	...
	secure := func(permission string, handler http.Handler) http.Handler {
		return withAccessLog(withAuth(gosightauth.RequirePermission(permission, handler, commandsHandler.Sys.Stores.Users)))
	}

	// Command execution routes
	r.Handle("/commands",
		secure("gosight:api:commands:execute", http.HandlerFunc(commandsHandler.HandleCommandsAPI))).
		Methods("POST")
}
